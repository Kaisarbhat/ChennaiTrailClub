...
const PrivacyPolicy = () => {
  const data = [
    ...
    },
    {
      key: 12,
      heading: "Contact",
      text: ["Should you have any questions about our privacy practices or this Privacy Policy, or if you would like to exercise any of the rights available to you, please email us at chennaitrailclub@gmail.com "],
    },
  ];
  return (
        ...
        <div className="w-full  mt-32 mb-16 text-[14px] text-[#50514C]">
          ...
          <div className="space-y-4 mt-8 tracking-tighter leading-loose font-man">
                     {data.map((item) => (
                        <di key={item.key}
                        >
                          <h3 className="text-[32px] text-[#070802] font-bold font-serif"> {item.heading}</h3>
                          {item.text.map((para) => (
                            <p className="mt-[10px]">{para}</p>
                          ))}
                        </di>
                      ))}
          </div>
