...
function UpcomingEventsHero({ data }) {
  ...
  return (
    <div className="overflow-clip bg-[#070802] xs:min-h-[100vh] sm:min-h-[71vh] md:min-h-[100vh] text-white box-border  flex flex-col  pt-12 md:px-4 font-serif items-center justify-evenly">
      <div className="flex md:flex-row xs:flex-col  md:space-x-8 xs:space-x-0 xs:text-center md:text-start max-w-[1340px]">
        ...
        <div className="flex flex-col xs:items-center md:items-start mb-10  md:mt-0 xs:mt-8 md:space-y-3 xs:space-y-0">
          <div className="md:text-[18px] xs:text-[14px]  text-[#ffffffbf]">
            <i className="bi bi-calendar3 text-[#D0F700] mr-1"></i>{" "}
            {formattedDate}
          </div>
          <motion.div
            initial={{ opacity: 0, y: "100%" }}
          ...
          >
            ...
            >
              <i className="bi bi-geo-alt-fill text-[#D0F700] mr-1 "></i>
              {eventData.location} <i className="bi bi-arrow-up-right"></i>
            </a>
          </motion.div>
          <Button
            title={"View Details"}
